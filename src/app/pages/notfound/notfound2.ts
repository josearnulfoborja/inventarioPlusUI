...
@Component({
    selector: 'app-notfound2',
    standalone: true,
    imports: [RouterModule, ButtonModule],
    template: `
        <div class="fixed w-full h-full top-0 left-0 opacity-70 text-primary bg-primary-contrast">
            <div class="flex items-center justify-center relative">
                <div class="w-full flex justify-end items-start absolute top-0 left-0 z-10"></div>
                <div class="h-screen flex justify-center items-center w-full lg:w-6/12">
                    <div class="z-50 w-6/12 h-6">
                        <span class="text-lg bg-slate-500 p-1 px-2 rounded-md text-surface-900 dark:text-surface-0">404</span>
                        <h1 class="text-6xl font-light my-6 mx-0 text-slate-500">Page Not Found</h1>
                        <div class="w-28 bg-slate-500" style="height: 1px"></div>
                        <p class="mt-6 text-lg text-surface-900 dark:text-surface-0">Requested resource is not available right now. Please try again later.</p>
                        <button pButton pRipple type="button" [routerLink]="['/']" label="Go To Dashboard" class="mt-8 !border-0" style="background-color: #5e81ac;"></button>
                    </div>
                </div>
