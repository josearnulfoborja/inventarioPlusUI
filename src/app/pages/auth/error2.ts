...
@Component({
    selector: 'app-error2',
    imports: [ButtonModule, RouterModule, DividerModule, AppConfigurator],
    standalone: true,
    template: ` <div class="fixed w-full h-full top-0 left-0 opacity-70 text-primary bg-primary-contrast">
            <div class="flex items-center justify-center relative">
                <div class="h-screen flex justify-center items-center w-full lg:w-6/12">
                    <div class="z-50 w-6/12 h-6">
                        <i class="pi pi-exclamation-triangle text-2xl bg-red-500 p-1 px-2 rounded-md text-white !leading-[2rem]"></i>
                        <h1 class="text-6xl font-light my-6 mx-0 text-red-500">Error Occured</h1>
                        <p-divider styleClass="w-28"></p-divider>
                        <p class="mt-6 text-lg text-color">Something went wrong.</p>
                        <button pButton pRipple type="button" [routerLink]="['/']" label="Go To Dashboard" class="mt-8 !border-0 !bg-red-500"></button>
                    </div>
                </div>
            </div>
